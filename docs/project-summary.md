...
![梧桐遇 CampusMeet 标志](../apps/web/public/campusmeet-mark.svg)
...
| 时间 | 演示内容 | 重点 |
|---|---|---|
| 0:00-1:00 | 项目背景、定位和品牌 | 说明为何不是普通论坛 |
| 1:00-2:00 | 登录、校园认证、首页 | 白名单内测与可信身份 |
| 2:00-3:00 | 探索正式活动和组队 | 内容分层、搜索筛选 |
| 3:00-4:30 | AI 对话式发布 | 一句话转结构化草稿 |
| 4:30-5:15 | 分类审核和自动封面 | AI 与后端规则分工 |
| 5:15-6:30 | 另一账号申请加入 | 结构化申请与通知 |
| 6:30-7:30 | 发布者接受申请 | 成员写入、会话创建 |
| 7:30-8:30 | 消息和确认组队 | 隐私保护与双向确认 |
| 8:30-9:15 | 团队空间和 AI 规划 | 分工、议程、任务 |
| 9:15-10:00 | 授权与管理中心 | 正式活动发布资格和治理体系 |

演示前建议依次唤醒后端健康检查、登录两个白名单账号、准备一个可申请的组队帖，并确认 Coze 工作流与邮件服务环境变量有效。

## 14. 测试与质量保障

### 14.1 测试结构

仓库当前包含 53 个后端测试文件和 40 个前端测试文件，覆盖范围包括：
...
| 原生客户端 | 当前为响应式 Web，没有 iOS/Android 原生应用 |
...
本文档以仓库代码基线 `96dc631` 为准。后续功能、部署地址或访问策略发生变化时，应同步更新文档版本和“当前边界”章节。
