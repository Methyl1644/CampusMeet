...
vi.mock('@/api/personal', () => ({ getPublicProfile: vi.fn() }))
...
describe('PostMatchPanel', () => {
  ...
  it('recommends teammates with score, reason and public profile detail', async () => {
    vi.mocked(matchPosts).mockResolvedValue({
      matches: [{ user_id: '2', score: 88, reason: '技能覆盖所需的前端开发角色' }],
    })
    vi.mocked(getPublicProfile).mockResolvedValue({
      id: '2', nickname: '林晓', avatar: null, bio: null, looking_for: [], is_owner: false,
      major: '计算机科学与技术', grade: '大二', skills: ['React', 'TypeScript'],
    })

    renderPanel()
    fireEvent.click(screen.getByRole('button', { name: /AI 匹配队友/ }))

    expect(await screen.findByText('林晓')).toBeTruthy()
    expect(screen.getByText(/技能覆盖所需的前端开发角色/)).toBeTruthy()
    expect(screen.getByText(/匹配度 88/)).toBeTruthy()
  ...
  it('never renders contact details from the public profile', async () => {
    vi.mocked(matchPosts).mockResolvedValue({
      matches: [{ user_id: '2', score: 75, reason: '专业相关' }],
    })
    vi.mocked(getPublicProfile).mockResolvedValue({
      id: '2', nickname: '林晓', avatar: null, bio: null, looking_for: [], is_owner: false,
      skills: [], contact: { wechat: 'linxiao_wx' },
    })
