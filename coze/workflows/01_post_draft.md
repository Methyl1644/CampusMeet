...
| Render 变量 | `COZE_WORKFLOW_POST_DRAFT` |
...
2. **输入整理**：去除 `message` 首尾空格并限制 2000 字；确认 `kind` 枚举；将空的 `draft/user_skills` 设为 `""`，空的 `topic_id` 设为 null。
...
6. **结束**：输出 Schema 中九个字段逐一映射，不能把整个模型文本作为一个字符串返回。
...
next_field = missing_fields[0]，若为空则 null
